...
public class Created : State<CardFSM> {
    ...
    public override void Enter(CardFSM fsm) {
        ...
        if (fsm.createAsDisable || !CardsDataV1.Instance.cards.Contains(fsm.cardId)) {
        ...
        if (fsm.showQuantity) {
            ...
            float maxQuantity = CardsDataV1.Instance.GetCardMaxQuantity(fsm.cardId);
...
public class NotFound : State<CardFSM> {
    public override void Enter(CardFSM fsm) {
        fsm.DestroyCardBox();
        fsm.components.boxCardNotFound.SetActive(true);
...
public class Found : State<CardFSM> {
    ...
    public override void SyncData(CardFSM fsm) {
        ...
        fsm.components.totalQuantity.text = cardQuantity.ToString();
