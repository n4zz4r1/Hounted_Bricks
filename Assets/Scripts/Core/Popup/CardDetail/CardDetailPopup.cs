...
public class CardDetailPopup : StateMachine<CardDetailPopup, State<CardDetailPopup>> {
    ...
    protected override void SyncDataBase() {
        components.currentRarityText.text = LocalizationUtils.GetLocalizedText("CardRarity." + CardFSM.Rarity);
    ...
    private void CardSetup(CardFSM cardFSM) {
        CardFSM = cardFSM;
        ...
        components.quantityMaxText.text = CardsDataV1.Instance.GetCardMaxQuantity(cardFSM.cardId).ToString();
        components.cardTypeText.text = LocalizationUtils.GetLocalizedText("CardType." + cardFSM.cardType);
